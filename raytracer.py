...
# Image check
im = Image.open("raytracer-files/ray-suns.png")
print(im.getpixel((50,40)))
        ...
        if t_closest != np.inf:
            ...
            if s_t_close != np.inf:
                ...
                if sun: 
                    ...
                    for sn in range(len(sun_colors)):
                        sun_d = suns[sn]
                        sun_d = sun_d / np.linalg.norm(sun_d)
                        light_combo += sun_colors[sn] * np.dot(normal, sun_d)
                    colors = np.array(colors) * light_combo
                else:
...
# Check pixel
print(image.getpixel((50,40)))
image.im.putpixel((50,40), (128,128,128,255))
